...
UE BP Copilot 网页会把你填写的模型接口地址、模型名、API Key 和本次请求发送到本机代理，再由本机代理请求模型接口。代理不会保存 API Key，也不会保存请求内容。复杂蓝图生成时，本地代理最多等待 300 秒；`chat/completions` 的纯 JSON 模式会尝试用上游 `stream: true` 收集完整结果，减少第三方网关长时间无输出导致的超时。
