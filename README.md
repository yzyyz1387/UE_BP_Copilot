...
UE 蓝图 AI 工作台可以通过浏览器直连、云端中转或本地代理三种方式连接 OpenAI 兼容接口，生成结构化蓝图方案。左侧是可折叠蓝图库，并在下方提供类似 UE “我的蓝图”里的变量面板，支持按用户和文件夹保存不同蓝图；中间是 UE5 风格节点画布；右侧是接口设置、导入、变量详情、提示、清单和聊天输入区。
...
- **UE 风格变量面板**：当前蓝图变量会显示在左侧下方，支持手动新增、编辑、删除，并同步写入 `plan.variables`。
...
当前蓝图的 `plan.variables` 会在左侧下方以 UE 风格列表展示。点击变量区右上角 `＋` 可以手动添加变量；点击变量行可以编辑名称、类型、默认值、实例可编辑、生成时公开和说明；点击 `×` 删除变量。所有修改都会立即同步到当前蓝图 JSON 和本地蓝图库。
